...
class PurchasesController extends Controller
{
    ...
    {
        $invoices = Invoice::WhereIn('type', ['purchases', 'purchases_return'])->paginate(10);
        ...
        $buttons = [
            'add' => route('admin.purchases.create'),
            'filter' => true,
            'back' => route('admin.dashboard'),
            'form' => route('admin.purchases.index')
        ];

        if (request()->keyword) {
            $keyword = request()->keyword;
            $invoices = Invoice::WhereIn('type', ['purchases', 'purchases_return'])->where('number', 'like', '%' . $keyword . '%')
                ->paginate(10);
        }
        foreach ($invoices as $invoice) {
            $invoice->supplier_name = $invoice->profile->name;
        }

        return view('admin.purchases.index', compact('invoices', 'headers', 'route', 'buttons'));
    }
    ...
    {
        $back = route('admin.purchases.index');
        $currencies = Currency::all();
        return view('admin.purchases.create', compact('back', 'currencies'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $lastInvoiceNumber = Invoice::where('type', 'purchases')->max('number');
    ...
    public function edit(string $id)
    {
        ...
        return view('admin.purchases.edit', compact('invoice', 'invoice_items' , 'back' , 'currencies' , 'country_options' , 'province_options' , 'area_options' , 'sub_area_options' ));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            return redirect()->route('admin.purchases.index')->with('success', 'Invoice updated successfully.');
        } catch (\Exception $e) {
            // Log the exception for debugging
    ...
    public function destroy(string $id)
    {
        ...
        $invoice->delete();
        return redirect()->route('admin.purchases.index')->with('success', 'تمت العملية بنجاح');
    }
    ...
    public function SearchSupplier(Request $request)
    {
        try {
            ...
            $profiles = Profile::where('type', 'supplier')
    ...
    public function print(string $id){
        ...
        return view('admin.purchases.print', compact('invoice' ,'invoice_items'));
    }
}
